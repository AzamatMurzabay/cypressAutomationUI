// Описание теста
describe("INTERCEPT", () => {
  // Тест с имитацией сетевого запроса
  it("network request spy", function () {
    // **1.** Загрузить фикстуру (ака. Тестовые данные) с данными прогресса курса
    cy.fixture("progress.json").as("data");
    // **2.** Имитировать POST-запрос на /login
    cy.intercept("POST", "*/login").as("login");
    // **3.** Имитировать GET-запрос на /course/coursesProgress/:id с данными прогресса из фикстуры (ака. Тестовые данные)
    cy.intercept(
      "https://server-stage.pasv.us/course/coursesProgress/65d4265ddb75721937e5329c",
    ...
    cy.visit("https://stage.pasv.us/profile/65d4265ddb75721937e5329c/progress");
    // **11. Дождаться завершения имитированного запроса course и сравнить ответ с данными из фикстуры**ы
